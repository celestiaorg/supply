...
func Test_inflationRate(t *testing.T) {
	...
	testCases := []testCase{
		...
		{cip41ActivationDate.Add(-1 * day), 0.046658}, // Before CIP-41 activation
		{cip41ActivationDate, 0.023242},               // CIP-41 activation
		{cip41ActivationDate.Add(1 * day), 0.023242},  // After CIP-41 activation
...
func Test_annualProvisions(t *testing.T) {
	...
	testCases := []testCase{
		...
		{TGE.Add(3 * year), 26277422542444},
		{TGE.Add(20 * year), 23880672243416},
	}
	for _, tc := range testCases {
		t.Run(fmt.Sprintf("time %v", tc.t), func(t *testing.T) {
